...
def scrape_linkedin_jobs(keywords, location):
    ...
    search_url = f"{linkedin_search_url}keywords={keywords}&location={location}&start=0"
    ...
    for job in jobs_on_this_page:
        try:
            ...
            company_name = job_soup.find("div", {"class": "top-card-layout__card"}).find("a").find("img").get("alt")
            job_title = job_soup.find("div", {"class": "top-card-layout__entity-info"}).find("a").text.strip()
            job_seniority = job_soup.find("ul", {"class": "description__job-criteria-list"}).find("li").text.replace("Seniority level", "").strip()
            job_link = job_soup.find("div", {"class": "top-card-layout__entity-info"}).find("a").get("href")
...
jobs = scrape_linkedin_jobs("Python", "New York")
for job in jobs:
    print(job)
