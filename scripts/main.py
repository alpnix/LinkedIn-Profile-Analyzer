...
def job_keywords(job):
    prompt = "Give me a list of technical and soft skills for a job role as a {} in the format of a Python array. Only return the array, no extra texts.".format(
        job
    )
    ...
    response = ast.literal_eval(response)  # turn string into list
    return response


def evaluate(profile_keywords, job):
    prompt = "I want to be a {}. These are some keywords of my experience: {}. How much does my experience keywords eval with the job's technical and softskills? What else should I learn? Return only a short, concise evaluation followed by a list of skills I need to learn in a Python list format.".format(
